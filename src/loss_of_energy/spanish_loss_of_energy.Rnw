...
ggplot(data = dummy, aes(x = x, y = y)) +
    ...
    annotate("text", label = label_other_sex_age, x = 7.75, y = -0.9, size = 7) +

    annotate("text", label = translate_rundown_spanish(pt_run_down_text), x = 2.3, y = -1.8, size = 6, fontface = 'italic') +
    annotate("text", label = translate_rundown_spanish(sex_age_run_down_text), x = 7.75, y = -2.5, size = 6, fontface = 'italic') +
