...
ggplot(data = dummy, aes(x = x, y = y)) +
    ...
    annotate("text", label = config_pt_bp_s, x = stoplight_s_number_pos$x, y = stoplight_s_number_pos$y, size = 12) +
    annotate("text", label = config_pt_bp_d, x = stoplight_d_number_pos$x, y = stoplight_d_number_pos$y, size = 12) +
    xlab(NULL) +
    ylab(NULL) +
