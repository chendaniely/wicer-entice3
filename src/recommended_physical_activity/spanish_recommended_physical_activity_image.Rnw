...
moderate_exercise_plot <- ggplot(data = df) +
    ...
    scale_y_continuous(expand = c(0,0), limits = c(0, max_y),
                       breaks = seq(0, max_y, plyr::round_any(max_y, 10) * 0.1)) +
...
<<results='hide'>>=
png(filename = 'src/recommended_physical_activity/moderate_exercise_plot.png', width = 900, height = 600)
...
\begin{figure}[H]
\includegraphics[scale=0.37]{moderate_exercise_plot}
\end{figure}
