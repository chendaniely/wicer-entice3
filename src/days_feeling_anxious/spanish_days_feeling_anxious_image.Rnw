...
\begin{center}
\begin{figure}[H]
\centerline{\includegraphics[scale=1.25, trim=30 40 20 30, clip=true]{anxious_30}}
\end{figure}
\Huge \textbf{Usted}\\
\Huge \textit{\Sexpr{config_pt_anxious_30 } días}\\
...
\begin{center}
\begin{figure}[H]
\centerline{\includegraphics[scale=1.25, trim=30 40 20 30, clip=true]{sex_age_anxious_30}}
\end{figure}
\Huge \Sexpr{get_men_women(config_pt_sex, 'spanish')} de la misma\\
edad que usted\\
\Huge \textit{\Sexpr{config_sex_age_anxious_30} días}
\end{center}
\end{minipage}
\end{table}
