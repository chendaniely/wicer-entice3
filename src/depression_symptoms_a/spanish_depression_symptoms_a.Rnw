...
\caption*{\large{\Sexpr{get_men_women(config_pt_sex, 'spanish')}\\
...
\Large{
Su nivel de síntomas de depresión fue \Sexpr{pt_stat}.\\

El nivel de síntomas de depresión de
\Sexpr{tolower(get_men_women(config_pt_sex, 'spanish'))}
de la misma edad que usted fue \Sexpr{other_stat}.
